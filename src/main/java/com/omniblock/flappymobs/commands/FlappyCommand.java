    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (sub.equals("flight")) {
            ...
            if (flightManager.isInFlight(player)) {
                player.sendMessage(messagesManager.getPrefixedMessage("already_riding"));
                return true;
            }
            if (args.length < 2) {
            ...
            if (!sender.hasPermission("fp.nocost") && flight.getCost() > 0) {
                if (!economyManager.hasBalance(player, flight.getCost())) {
                    player.sendMessage(messagesManager.getPrefixedMessage("insufficient_funds", "cost", economyManager.formatAmount(flight.getCost())));
                    return true;
                }
                economyManager.withdraw(player, flight.getCost());
                player.sendMessage(messagesManager.getPrefixedMessage("payment_success", "cost", economyManager.formatAmount(flight.getCost())));
            }
        ...
        if (sub.equals("list")) {
            ...
            for (Flight flight : flightManager.getAllFlights()) {
                sender.sendMessage(messagesManager.getMessage("list_entry", "flight", flight.getName(), "creature", flight.getCreature().name(), "cost", economyManager.formatAmount(flight.getCost())));
            }
        ...
        if (sub.equals("create")) {
            ...
            double cost = 0;
            if (args.length > 3) {
                try {cost = Double.parseDouble(args[3]);} catch (Exception ignored) {cost = 0;}
            }
            EntityType type;
            try {type = EntityType.valueOf(mob);} catch (Exception e) {player.sendMessage(messagesManager.getPrefixedMessage("invalid_creature"));return true;}
            if (!plugin.getConfigManager().isCreatureEnabled(type)) {player.sendMessage(messagesManager.getPrefixedMessage("creature_disabled"));return true;}
            if (flightManager.getFlight(name) != null) {player.sendMessage(messagesManager.getPrefixedMessage("create_exists"));return true;}
        ...
        if (sub.equals("save")) {
            ...
            player.sendMessage(messagesManager.getPrefixedMessage("save_success", "flight", creating.getName(), "waypoints", String.valueOf(creating.getWaypoints().size())));
            return true;
        }
        ...
        if (sub.equals("edit")) {
            ...
            switch (property.toLowerCase()) {
                case "creature":
                    try { f.setCreature(EntityType.valueOf(value)); }
                    catch (Exception e) { sender.sendMessage(messagesManager.getPrefixedMessage("edit_invalid_value", "property", property)); return true; }
                    break;
                case "cost":
                    try { f.setCost(Double.parseDouble(value)); }
                    catch (Exception e) { sender.sendMessage(messagesManager.getPrefixedMessage("edit_invalid_value", "property", property)); return true; }
                    break;
                case "invulnerable":
                    ...
                    break;
                case "parachute":
                    try { f.setParachuteTime(Integer.parseInt(value)); }
                    catch (Exception e) { sender.sendMessage(messagesManager.getPrefixedMessage("edit_invalid_value", "property", property)); return true; }
                    break;
                default:
            ...
            sender.sendMessage(messagesManager.getPrefixedMessage("edit_success", "property", property, "flight", name, "value", value));
            return true;
        }
